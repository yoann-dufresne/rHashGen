...
int main()
{
    ...
    SamplingAvalancheTest<myuint> sampling_test{value_size};
    CLUTCHLOG(progress, "Run SoftAvalancheTest");
    for (size_t i = 0; i < 20; i++)
    {
        sampling_test.set_hash_function(hashFunc);
        CLUTCHLOG(note, "     10 000 iterations:\t" << sampling_test(value_size * 10000UL));
    }
    std::cout << std::endl;
    ...
    {
        sampling_test.set_hash_function(hashFunc);
        CLUTCHLOG(note, "    100 000 iterations:\t" << sampling_test(value_size * 100000UL));
    }
    std::cout << std::endl;
    ...
    {
        sampling_test.set_hash_function(hashFunc);
        CLUTCHLOG(note, "  1 000 000 iterations:\t" << sampling_test(value_size * 1000000UL));
    }
    
    sampling_test.set_hash_function(hashFunc);
    CLUTCHLOG(note, "100 000 000 iterations:\t" << sampling_test(value_size * 100000000UL));
    ...
    combi::EvalFull<myuint,Combi> eval(value_size, forge, sampling_test);
